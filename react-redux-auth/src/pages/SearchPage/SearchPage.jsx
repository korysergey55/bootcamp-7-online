...
import stringifyQuery from "../../lib/qs/stringify-query";
...
class SearchPage extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (query !== currentQuery || prevState.page !== this.state.page) {
      this.handleSearchProducts();
    }
  }
  ...
  handleSearchProducts = async () => {
    ...
    try {
      const response = await axios.get(
        `https://amz-app.herokuapp.com/api/v1/products?search=${query}&limits=${3}&page=${page}`
      );
  ...
  handleSubmit = (search) => {
    ...
    const queryStr = stringifyQuery({ query: search, page: 1, limits: 5 });
